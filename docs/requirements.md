...
| FR-E8 | MUST | **A notice and action mechanism exists**: anyone, signed in or not, can report a contribution they consider illegal or harmful. DSA Article 16 binds every hosting provider regardless of size, and Article 19's micro-enterprise exemption does not cover it. Nothing in the product implements this yet, which makes it the first piece of moderation work. **[VERIFIED]** François, 2026-09-13. `design/moderation.md` 0. |
...
| FR-E10 | MUST | **Nothing is ever removed automatically.** Removal is always a human act. A report threshold that removes content is a brigading tool, and what it would remove is precisely the argued negative contribution this platform exists to protect. **[VERIFIED]** François, 2026-09-13, resolving OPEN-24. |
| FR-E11 | MUST | **Holding is automatic, reversible, and narrow.** Automated checks may hold, never remove, and only on dull signals: a third party's name, contact details, a URL. Never sentiment or a quality score, because a classifier that holds "too negative" contributions is a censorship machine wearing an algorithm. A single report does not hold; a report alleging illegality or naming a third party holds immediately, because the cost of holding a good contribution for a day is far below the cost of knowingly hosting a defamatory one. **[VERIFIED]** François, 2026-09-13. |
...
| FR-G7 | MUST | **The chosen language holds everywhere, including inside a module.** One selection, applied to every zone, every module and every control: a screen whose buttons are French while its text is English tells a Dutch or English speaker the product was not built for them. Enforced by `test/architecture/translated.test.ts`, which scans the components for prose sitting in the markup rather than checking that the catalogues agree, because the failure it exists for was 125 sentences that never became keys at all. **[VERIFIED]** François, 2026-09-13, reporting French buttons with `en` and `nl` selected. |
...
| ID | Priority | Requirement |
|---|---|---|
| FR-H1 | MUST | **Notifications exist**, to the address in FR-A12. **[VERIFIED]** François, 2026-09-13: "The notification system is really important for me." What they carry is decided per kind, and each kind is a preference of its own: one switch labelled "notifications" is how people end up unsubscribing from the one thing they wanted in order to escape the rest. |
