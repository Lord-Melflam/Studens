...
| OPEN-48 | **A course code in a URL does not say which university it belongs to.** `/app/ryc/c/lepl1503` resolves a code against the whole catalogue, which is unambiguous with one institution loaded and ambiguous with two. Codes are now scoped in the database (20260918_codes_belong_to_an_institution), so the collision is recorded rather than silently resolved, but the read path still takes the first match by institution id. Options: put the institution in the path (`/c/ulb/comm-b1010`), which breaks existing links; keep bare codes and disambiguate only on collision, which makes one URL mean different things over time; or make the reader institution-scoped and choose the institution elsewhere. **Blocks loading a second catalogue**, not the schema change. | FR-D16, FR-D25, `ref/read.ts` |
