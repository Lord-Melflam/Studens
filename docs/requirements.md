...
- **[VERIFIED]** stated directly by the owner, or observed in a system of record.
  Authoritative.
...
**RYC is a prototype, not a specification.** It was drafted many months before
2026-09-09, never finished, and it does not work. It is described by its author as a
context refining experiment. **[VERIFIED]** 2026-09-09.
...
`[VERIFIED]` once the owner confirms it. Until then it stays `[PRIOR-ART]`.

## 1. Purpose and scope

### 1.0 Long-term vision

**[VERIFIED]** 2026-09-09. This is direction, not v1 scope. See 1.3.
...
are observed; the list below is what exists today, not a plan. **[VERIFIED]**,
2026-09-10.
...
**[VERIFIED]** 2026-09-10, for both names and the ordering.
...
progresses, on the belief that AI used properly makes things better. **[VERIFIED]**,
2026-09-10.

It means **product features, not developer tooling**. **[VERIFIED]** 2026-09-10,
...
Stated at the outset and unchanged since. They are in the specification
...
**[VERIFIED]** 2026-09-10. Resolves OPEN-1.
...
peers. **[VERIFIED]** 2026-09-10, first-hand as an EPL master's student.
...
Modules are self-contained functional areas, basic or advanced. **[VERIFIED]**,
2026-09-09.

**RYC** ("Rate Your Courses"), a course review tool where **anonymity is an option and not
the default** (3.3), is the intended first module as a *concept*. **[VERIFIED]**,
...
**[VERIFIED]** 2026-09-10, for the whole list. Resolves OPEN-2.
...
**[VERIFIED]** 2026-09-10. Resolves OPEN-14 and OPEN-11.
...
**CHANGED 2026-09-17: v1 serves UCLouvain and ULB.** These are the two universities to
start with: both hold likely testers, and the added value of covering them is large.
...
**[VERIFIED]** 2026-09-10, after reviewing the concrete schema. Full comparison and
...
| FR-A3 | MUST | Sessions expire after **14 days idle** and after **90 days absolute**, whichever comes first. **[VERIFIED]** 2026-09-10. Resolves OPEN-4. |
...
| FR-A6 | MUST | Registration is **open to the public**. No institutional gating, no roster check, no invitation. **[VERIFIED]** 2026-09-10. Resolves OPEN-3. |
| FR-A7 | MUST | Authentication is **OAuth only**: Microsoft and Google. **No self-managed credentials**, so no password storage, no reset flow and no password breach surface. **[VERIFIED]** 2026-09-10. Resolves OPEN-5. |
...
| FR-A9 | MUST | A Member's email **domain** may be recorded as a trust signal and shown on attributed contributions. It must never render on an anonymous one (FR-C16). The domain is taken from the **verified claim in the provider's token**, never from user input. Superseded in part by FR-A11: this says what the domain is FOR, and no longer says the address is discarded. |
...
| FR-A11 | MUST | **The email address is stored.** Reversing the domain-only reading of FR-A9, which was an assumption here and not a requirement. **[VERIFIED]** 2026-09-13: the address is used at sign-in anyway, so refusing to store it serves no purpose provided the GDPR obligations are met. The provider already sends it on every sign-in, so refusing to keep it bought no privacy and cost the product every feature that needs to reach a person: notifications, an account deletion someone can confirm, and any contact at all. Lawful basis is Article 6(1)(b), performance of the contract with the member, for the address attached to the account, and 6(1)(a) consent for anything optional sent to it, recorded per purpose and withdrawable in one click. Alternatives rejected: a hash of the address, which can be sent nothing; and asking for an address separately at first use, which is the same processing with worse completion. Costs accepted and NOT to be minimised: an address is personal data under the GDPR where a domain arguably was not, so retention, rectification, erasure, export and breach notification all now apply to it, and a breach of this table becomes reportable. What would change the answer: nothing short of dropping every feature that needs to reach a member. |
| FR-A12 | MUST | **The identity address and the contact address are separate columns.** `providerEmail` comes from the verified token and is never editable, because it is how the same person is recognised on the next sign-in. `contactEmail` is what the member chooses to be reached at, defaults to the provider's, and can be changed or emptied. Rejected: one editable column, which would let somebody edit the field their identity is matched on and either lock themselves out or take over another account. The domain in FR-A9 keeps coming from `providerEmail` alone, so a self-declared contact address can never raise a trust signal. |
| FR-A13 | MUST | **A changed contact address is confirmed before anything is sent to it**, by a signed, single-use, time-limited link sent to the new address. Without it, a typo silently redirects a member's mail to a stranger, and an attacker with a borrowed session redirects it deliberately. The old address is told that the change was requested, which is what makes the takeover visible to the person losing it, and that warning carries no link: "click here if this was not you" is the shape every phishing message takes, and it is being read by somebody who may be losing their account. **The link lasts 24 hours.** It was one hour until 2026-09-13, on the reasoning that a leaked link should rot quickly; that reasoning weighed the wrong risk. An hour barely shrinks the window for an attacker who already holds the message, and badly shrinks it for a person who reads their mail the next morning. **[VERIFIED]** 2026-09-13, after every queued confirmation expired before it could be used. Cost accepted: a link in a shared or compromised mailbox is usable for a day. Bounded by what it can do, which is set the CONTACT address alone (FR-A12): it signs nobody in and cannot touch the identity address. What would change the answer: giving the token any authority beyond that. **Single-use is enforced without stored state**: the token carries a fingerprint of the contact state it was issued against, and applying the change moves that state, so a replay no longer matches. That was found by using it: on 2026-09-13 a two-hour-old link was replayed three times against a live account and applied every time, overwriting an address the member had since set. The word "single-use" had been dropped from this requirement while rewriting it, which is how a guarantee stops being one. The duration is stated in the message in three languages, and a test pins those to the constant, because a message promising a lifetime the code does not honour is the same failure as a screen claiming a message was sent. |
| FR-A14 | MUST NOT | **No password reset, because there are no passwords.** FR-A7 removed self-managed credentials, so there is nothing to reset and offering it would imply a credential we deliberately do not hold. What a member can actually lose is access to their Microsoft or Google account, and that is recovered with the provider, not with us. Where that is permanently lost the account is not recoverable, and FR-A15's export is what limits the damage. **[DERIVED]** 2026-09-13, answering the request for a reset flow with the thing it was asking for. |
| FR-A15 | MUST | **A Member can delete their account, and export what is held about them**, both without asking anybody. GDPR Articles 17 and 20, and also the plainer point that an account you cannot leave is a product people do not join. Deletion removes the Member row and everything keyed to it, and revokes every session. **It does not remove anonymous contributions, because it cannot**: there is no column joining them to anybody (FR-C2), which is the guarantee working rather than failing, and FR-C9 already says so in advance. Attributed contributions are **detached**: the text stays and the name goes, which is OPEN-46 resolved 2026-09-13, and a fourth state rather than a second anonymous one. Deletion is confirmed, is preceded by a plain statement of what will and will not disappear, and is irreversible. |
...
**[VERIFIED]** 2026-09-10. Resolves OPEN-5.
...
| FR-B14 | MUST | **All module code lives in this repository and is admitted by review before merge.** No runtime loading of third-party code, no plugin mechanism, no module registry. **[VERIFIED]** 2026-09-10. Resolves OPEN-16. |
| FR-B16 | MUST | **The frontend has the same tiers as the backend.** `apps/web` is the SHELL: navigation, and a registry of the modules a Member may use (FR-B1). A module's user interface lives in the module's own package, not in the shell. **[VERIFIED]** 2026-09-10. |
...
| FR-B21 | MUST | **What is on screen is in the address, and what is not published is not.** Every part of what a visitor is looking at goes in the URL: the screen, the thing being looked at, and the screen's own settings, meaning filters, sort orders and what has been typed into a search box. Held in component state instead, three things fail and the third fails silently: Back cannot restore what was never written down, a refresh cannot either, and a link to a filtered list shows its reader a different list. A change of screen pushes a history entry; a change of settings **replaces** it, or Back walks backwards through the visitor's own filtering instead of leaving the screen. Settings that no longer exist are dropped on arrival rather than left to empty the page with nothing to unclick. **The exclusion is not decoration**: nothing unpublished goes in a URL, above all the text of a contribution being written, because browser history, bookmark sync and whoever can see the screen all read a URL and FR-C9 reaches none of them. The shell carries the query string without reading it, exactly as it carries the path (FR-B16, FR-B18). **[VERIFIED]** 2026-09-17, after reporting that opening a course and pressing Back lost the filters. Raised as a general way of working, not as one screen's bug. |
| FR-B19 | MUST | **Nothing reaches `main` except through a pull request** that a second person approved and that both CI jobs passed. Decided 2026-09-11, when the second developer joined and mutual review became possible for the first time. Short lived branches off `main`, squash merge by default so `main` carries one commit per reviewed unit, linear history, no force pushes. Git flow was rejected: `develop` and release branches exist to support several released versions at once, and this project has one environment and no releases, so it would cost two merges per change for nothing. Direct commits to `main` were rejected because they defeat FR-B14. Cost accepted: `main` can break briefly if a bad change passes the gates, since there is no staged integration branch. What would change it: supporting more than one deployed version at a time. Workflow written out in `docs/CONTRIBUTING.md`. |
| FR-B20 | MUST | **Changes to a guarantee, or to an instrument that proves one, need the owner's review**, listed in `.github/CODEOWNERS` and enforced by branch protection. Two kinds of path qualify: things that *are* a guarantee (the schema and migrations, the grant matrix, the kernel, the two submission paths, the read path that strips author attributes) and things that *verify* one (the isolation script, the architecture and kernel tests, the lint boundary rule, the CI workflow). The second kind is listed because `LESSONS.md` section 9 records that the instrument is the part taken on trust. Cost accepted: one person is a bottleneck on those paths. Accepted because an error there publishes rows that can never be unlinked or recalled. What would change it: a third developer who knows FR-C well enough to review the kernel. Resolves the governance half of OPEN-18. |
| FR-B15 | MUST | Because FR-B14 makes review the security boundary, the automated gates (FR-B6, FR-C8, and the schema tests in the design notes) must pass regardless of who authored the change, including the project owner. A gate that can be bypassed by the author is not a control. **Enforced 2026-09-11** by branch protection on `main`: `gates` and `database` are required checks, one approval is required from someone other than the author, approvals are dismissed on new commits, and force pushes and deletions are refused. **With one recorded deviation:** administrators are exempt, by Decided on 2026-09-11, so the project owner can still push directly to `main`. The requirement is not weakened to match, because the gap is real and stating it is worth more than a document that agrees with itself. What it costs: on the owner's own changes, review is self-review again, which is the condition FR-B15 was written to compensate for. What would close it: setting `enforce_admins` to true, or a ruleset that keeps the required checks unbypassable while allowing an audited admin bypass of the approval alone. |
...
**[VERIFIED]** 2026-09-10, raised as "a user won't directly drop to the ryc in the
...
**[VERIFIED]** 2026-09-09. Resolves OPEN-12.
...
given review. **[VERIFIED]** 2026-09-10. Resolves the default half of OPEN-22.
...
Recorded here because it has been flagged as an open worry, avoiding bad surprises, and
it has a precise form. **[DERIVED]**, 2026-09-10, from the default decision above. Not
...
the candidate set. **[VERIFIED]** 2026-09-10, Option 1: the rule is enforced on the
...
| FR-C9 | MUST | An anonymous contribution cannot be edited or deleted **by its contributor**, ever. This follows from FR-C2: nobody can prove authorship, including its author. **[VERIFIED]** 2026-09-09. |
...
| FR-C13 | MUST | Per course uniqueness (FR-D9) is enforced on the **attributed** path only. On the anonymous path the quota (FR-C4) is the only limit. **[VERIFIED]** 2026-09-10, Option 1. |
| FR-C14 | MUST | An **attributed** contribution can be edited by its author. Contrast FR-C9, which is not a policy choice but a structural consequence. **[VERIFIED]** 2026-09-10. Resolves OPEN-22. |
| FR-C15 | MUST | Whether a contribution is anonymous or attributed **is** publicly visible: attributed shows the author, anonymous shows an explicit badge. **[VERIFIED]** 2026-09-10. Resolves OPEN-21. |
...
| FR-C21 | MUST | **Before** a contributor commits to anonymity, they are shown the counts that bear on their exposure: how many attributed and how many anonymous contributions the target already has. Where attributed participation is high and anonymous is zero, this is presented as a warning, not a statistic. Extends FR-C11. **[VERIFIED]** 2026-09-10. Part of the OPEN-19 resolution. |
...
| FR-C24 | MUST | **The quota bounds accounts, not people, and the privacy statement says so.** Registration is open (FR-A6), so one person can hold several accounts and neither the quota (FR-C4) nor per-course uniqueness (FR-D9) is an integrity guarantee. No friction is added at account creation: a CAPTCHA is bypassed for a few cents at scale and an email loop circles a mailbox the provider already verified, so both cost an ordinary user a step and the attacker nothing. Holding contributions from unrecognised email domains was rejected outright: it penalises exactly the people anonymity exists for, and edges toward treating a domain as proof of enrolment (FR-A10). **[VERIFIED]** 2026-09-13, resolving OPEN-35. `design/account-integrity.md`. |
| FR-C25 | MUST | **Manipulation is detected on the target, never on the author.** A burst of contributions to one course in a short window is a fact about the course, so measuring it needs no link between a member and what they wrote, which is the only reason this control survives FR-C2. It holds the burst for a human (FR-E11) and stores nothing on the contribution. Cost accepted: a patient attacker spreading contributions over months is not detected. **[VERIFIED]** 2026-09-13. |
...
**The threat model boundary, decided 2026-09-09.** **[VERIFIED]**. Rate limiting
...
**[VERIFIED]** 2026-09-10. Resolves OPEN-13. The v1 / v2 split is the owner's call of
the same date: build the core, defer the analytics.
...
| FR-D8 | MUST | A review carries **review text**, with a minimum length of **10 characters**, the same on both paths. Its purpose is to block an empty box, not to mandate an essay. **[VERIFIED]** 2026-09-10, ~~about 80 characters~~ **REVISED 2026-09-21 to 10**. The requirement did not change; the floor was the wrong instrument for it. A length rule cannot block a non-review: eighty characters of one phrase repeated clears it, and a complete short judgement does not, so it refused the honest short reviewer and admitted the lazy long one. What sorts a contribution is moderation (FR-E8), which exists, and the three numbers that travel beside the text on the named path, so a short sentence never arrives alone. **Alternatives:** keeping 80 and accepting the refusals, rejected because the first cohort meets an almost empty catalogue (OPEN-42) and every turned-away contribution costs more there; and removing the floor entirely, rejected because the field is required and an accidental submission is permanent on the anonymous path (FR-C9), which nothing can take back. **Cost accepted:** course pages will carry thin reviews, and the only answer to a thin one is a moderator or a reader's own judgement. **What would change the answer:** thin reviews arriving faster than moderation can read them, which is a volume problem and wants a different tool than a character count. Resolves OPEN-37. Also bounded **above**, at 4,000 characters, on the body and the advice alike. Added 2026-09-10: the only ceiling was the API's 32 kB request cap, which is a transport limit, so an over-long review failed the whole request with no field named after the person had written all of it. A limit that produces a field error is a different thing from one that produces a 413. |
...
| FR-D13 | MUST | **Course pages are public; reviews require a session to read.** Code, title, ECTS, description and lecturer are UCLouvain's own published data and stay open. Reviews do not. **[VERIFIED]** 2026-09-10, delegated decision. |
| FR-D14 | MUST | Review text is excluded from search engine indexing. |
| FR-D15 | MUST | On the **anonymous** path, a review displays its text and date only. Recommendation, workload and difficulty contribute to the aggregate but are **not shown per review**. The value of those numbers is the aggregate; a per-review triple plus prose is a detailed fingerprint on a record meant to be unlinkable. **[VERIFIED]** 2026-09-10. Part of the OPEN-19 resolution. |
...
| FR-D19 | MUST | **Assessment structure is scraped, never asked.** The catalogue publishes the evaluation method with weightings and the official contact hours, so a course page shows them from the reference module. Reviewers are asked only for what the catalogue cannot know. **[VERIFIED]** 2026-09-10. Verified against `cours-2025-lepl1503`. |
| FR-D26 | MUST | **A scraped field keeps the structure the source gave it.** The long course-page fields are lists, not prose: measured across 546 pages, 4,040 list items, 3,988 line breaks, 818 `ul`, 81 `ol`, nesting three deep. They are stored and rendered as structured blocks, so a thirty item list reads as a list. Structure is **preserved, never invented**: a field written as one paragraph stays one paragraph. **[DERIVED]** 2026-09-10, after Reported as the fields rendering in one unbroken blob. Model and measurements in `packages/ref/src/ingestion/parse/rich.ts`. |
...
| FR-D33 | MUST | **Both lists are filterable, and every facet is derived from the data in front of it.** The programme list filters on kind (bachelier, master, mineure, filière, and so on), site and text; the course list on term, credits, teaching language, the entity in charge, text, and whether anything has been written about it. Two rules make a facet trustworthy and neither is optional. **The options offered are the values present**, never a written list, for the same reason the faculty and programme structure is discovered at runtime: a fixed list is wrong within a year and offers options that match nothing. **Each facet's count is computed with its own dimension ignored and every other applied**, so a count is what selecting it would leave; counting against the unfiltered list instead produces a chip saying 23 next to a list that empties when pressed. A dimension with fewer than two values draws no control. Filtering is client side over data already fetched, because a programme holds at most a few hundred courses; the cost accepted is that this must move to the server the day a list is paged, and the function signatures are already the ones such an endpoint would take. **[DERIVED]** 2026-09-13, from FR-D24: discovery over 546 courses without filters is a scroll. |
| FR-D34 | MUST NOT | **No filter, sort or search on a lecturer's name.** Section 5.1 is that the GDPR exposure of this product is the lecturers, not the students: a review naming one processes a third party's personal data with no consent. A control that gathers one person's courses, and thereby everything written about them, in a single press is qualitatively different from a course page that happens to name who teaches it, and it is the feature a lecturer would point at. Alternatives rejected: allowing it behind a sign-in, which changes who can do it and not what it is; and allowing it for course discovery only, which is the same query with a different label. Cost accepted: "which courses does X teach" is a real student question and the official catalogue answers it. What would change the answer: the qualified reader in 5.1 concluding otherwise, in writing. **[DERIVED]** 2026-09-13. |
| FR-D20 | MUST | **Numbers describe the course. Prose discusses the teaching.** No numeric or categorical field rates an identifiable person. Teaching quality is expressed in review text only, and never as a score. **[VERIFIED]** 2026-09-10. Reason in 3.4. |
| FR-D21 | MUST | A reviewer **declares that they completed the course** before submitting. Required, and it blocks the review if not. Self-declared and unverifiable, consistent with FR-A6's good-faith model. Phrased as "completed" rather than "sat the exam", because some courses have no exam. Adopted from the EPL document's own rule. **[VERIFIED]** 2026-09-10. |
| FR-D22 | COULD | A reviewer **may** state whether they passed or failed. **Optional.** **[VERIFIED]** 2026-09-10, overruling a recommendation that it be required. Resolves OPEN-44. |
...
Listed because they were specified and deliberately set aside, not forgotten. The
owner's call, 2026-09-10.
...
**Pass or fail: optional, and never shown per review.** The owner's call, 2026-09-10. The
recommendation was to require it; optional was chosen, accepting the consequence, which is
...
| FR-E4 | MUST | Contributions are screened automatically before publication. **[VERIFIED]** 2026-09-09. |
...
| FR-E8 | MUST | **A notice and action mechanism exists**: anyone, signed in or not, can report a contribution they consider illegal or harmful. DSA Article 16 binds every hosting provider regardless of size, and Article 19's micro-enterprise exemption does not cover it. Nothing in the product implements this yet, which makes it the first piece of moderation work. **[VERIFIED]** 2026-09-13. `design/moderation.md` 0. |
| FR-E9 | MUST | **Removing a contribution publishes a statement of reasons in the place it occupied**, rather than sending one to its author. DSA Article 17 requires informing the affected user; on the anonymous path there is nobody to inform, and that is FR-C9 working rather than a gap. Publishing it is the only channel that exists, is far harder to abuse quietly than a silent deletion, and is the only redress an anonymous author can have, since they can contest a public reason where they could never prove authorship to contest a private one. **[OPEN]** This is a non-lawyer's reading and must be confirmed with 5.1. |
| FR-E10 | MUST | **Nothing is ever removed automatically.** Removal is always a human act. A report threshold that removes content is a brigading tool, and what it would remove is precisely the argued negative contribution this platform exists to protect. **[VERIFIED]** 2026-09-13, resolving OPEN-24. |
| FR-E11 | MUST | **Holding is automatic, reversible, and narrow.** Automated checks may hold, never remove, and only on dull signals: a third party's name, contact details, a URL. Never sentiment or a quality score, because a classifier that holds "too negative" contributions is a censorship machine wearing an algorithm. A single report does not hold; a report alleging illegality or naming a third party holds immediately, because the cost of holding a good contribution for a day is far below the cost of knowingly hosting a defamatory one. **[VERIFIED]** 2026-09-13. |
| FR-E12 | MUST | **Report counts are shown to a moderator and never acted on alone.** The number of reports is evidence about the reporters as much as about the content. Many reports from accounts created the same week is a brigading signature, and noticing it needs no link between a member and a contribution: it is a fact about the reports. **[DERIVED]** 2026-09-13. |
| FR-E13 | MUST | **No moderation response time is published until one has been measured.** The legal standard in DSA Article 6 is acting expeditiously once you have actual knowledge, which names no number; naming one ourselves creates an obligation the law did not impose, and a missed published promise is evidence against us at the moment someone is complaining. What is published is what happens: reports are read and acted on as fast as we can, and anything plausibly illegal or naming a third party is held first. Measured from the first day via FR-E1 and published once it is a fact. **[VERIFIED]** 2026-09-13, resolving OPEN-7. |
| FR-E14 | MUST | **A Moderator is appointed by an Administrator, one at a time, and the appointment is recorded** in the audit log naming the administrator (FR-B12 permits naming an actor there because the actor is a moderator). No self-service, no election, no promotion by tenure or reputation: each is a way to acquire the power without a human granting it, and a reputation system is gameable by exactly the person you least want moderating. **A Moderator cannot appoint a Moderator.** Powers are content powers only: hold, publish, remove with a reason. Never reading authorship, never sanctioning a person on the anonymous path (FR-E7). Cost accepted: it does not scale and makes the owner a bottleneck on recruiting, which is the right shape at this size. **[VERIFIED]** 2026-09-13, resolving the appointment half of OPEN-8. |
| FR-E15 | MUST | **A suspended account is told, in two ways, and both state the reason.** A message goes out the moment the decision is taken, to a confirmed contact address (FR-A13), carrying the reason in the moderator's own words, the end date or that there is none, and an address to write to. And the next sign-in shows a screen saying the same thing, because an account with no confirmed address would otherwise learn nothing. Two paths because each fails alone: somebody who gives up after one refused attempt never reaches the screen, and most accounts have no confirmed address. The message is **transactional, never a preference**: it is the account itself being stopped, like a deletion receipt, and making it opt-in means the people who never opened the notification screen find the door locked with no explanation. Alternatives rejected: refusing the sign-in like any other failure, which is what shipped and is indistinguishable from an outage, so a person reasonably concludes the site is down and keeps trying; and a message saying "sign in to find out why", which sends somebody to the one place they have just been shut out of and reads like phishing. Cost accepted: the reason travels by mail, so a badly worded one is not retractable once sent. What would change it: nothing short of a legal reason not to state grounds. **[DERIVED]** 2026-09-19. |
| FR-E16 | MUST | **Who is suspended is a register a moderator can read, not a fact only the act knows.** Current suspensions are listed with the reason, the end date, who decided, and whether a message could be sent, paged and searchable by name. Without it the only way to learn whether somebody was already suspended, or what for, was to suspend them again and read the answer back, which is a power with no record of its own use. Only current ones are listed: an expired suspension is over, and a permanent list of everybody ever stopped is a different and worse thing to keep; what happened stays in the audit log (FR-E1), which is where a record of the past belongs. The deciding administrator is read from that log rather than stored on the account, so there are not two answers that can disagree. Cost accepted: one more screen to keep correct, and a list that grows. **[DERIVED]** 2026-09-19. |
| FR-E17 | MUST | **A demoted Administrator has a recorded way back.** Administrators are peers: the only two refusals are changing your own role and demoting the last one, both existing to prevent the single state that needs a database to undo, and neither about seniority. So with two of them each can demote the other, which is ordinary and stays. What was not ordinary is that the demoted one had no route left but editing a row by hand, making an ordinary disagreement between two administrators irreversible in the product. `npm run admin -- <username> --force` appoints past the bootstrap's check and records it against the operator, exactly as the bootstrap does. **It grants no authority that was not already present**: running it needs the database, and anybody holding the database can write any row, so the choice is between a recorded, repeatable command and a silent UPDATE. Alternatives rejected: protecting the first Administrator from demotion, which hardcodes a privileged identity into a design whose mechanism is roles, and which today would enshrine the development account that cannot even sign in once `NODE_ENV=production`; and a fourth `owner` role above Administrator, which is a role for a set of one and a fourth power where the table says roles grow at the third. Cost accepted: anybody with shell access can restore their own administrator role, which is true of anybody with shell access regardless. What would change the answer: administrators who are not also operators of the machine, at which point the asymmetry is real and an `owner` role earns its place. **[DERIVED]** 2026-09-21. |
...
open weight models, to detect defamation and similar categories. **[VERIFIED]**,
2026-09-09.
...
| FR-F11 | MUST | An institution is shown by **name and colours, never by its mark**. Consistent with `docs/typeset/frontend-design.tex` 2.3: the marks are trademarked whatever their copyright status, EPL's CC BY-SA fights our own licence, and a grid of university logos on a sign-up page is the sharpest possible implied endorsement. **[VERIFIED]** 2026-09-12. Revisit only with written permission. **City struck 2026-09-16**: an institution does not have one. 268 of UCLouvain's 692 programmes for 2025-2026 are taught outside Louvain-la-Neuve, across eight campuses, and the screen was stating the opposite. The column is dropped rather than left empty, because ULiege names Gembloux and Arlon beside Liege and a column that must stay null everywhere it is not wrong is one the next person fills in. The site is a fact about a PROGRAMME and belongs in the catalogue; see `design/catalogue-ingestion.md` 10. |
| FR-F12 | MUST | The picker shows **every listed institution** and allows only those whose catalogue is ingested to be chosen. Today that is UCLouvain alone. The unavailable ones must be labelled with the reason, since a grid of disabled cards otherwise reads as broken. **[VERIFIED]** 2026-09-12. |
...
| FR-G7 | MUST | **The chosen language holds everywhere, including inside a module.** One selection, applied to every zone, every module and every control: a screen whose buttons are French while its text is English tells a Dutch or English speaker the product was not built for them. Enforced by `test/architecture/translated.test.ts`, which scans the components for prose sitting in the markup rather than checking that the catalogues agree, because the failure it exists for was 125 sentences that never became keys at all. **[VERIFIED]** 2026-09-13, reporting French buttons with `en` and `nl` selected. |

### 3.8 Reaching a member (FR-H)

Studens is the platform and RYC is one module of it. FR-C's guarantees are about
**contributions**, which are a module's concern; they were allowed to govern
decisions about the **account**, which is the platform's, and that is how the
product ended up unable to email anybody. FR-H is written at the platform's
level, and a module never sees an address.

| ID | Priority | Requirement |
|---|---|---|
| FR-H1 | MUST | **Notifications exist**, to the address in FR-A12. **[VERIFIED]** 2026-09-13: "The notification system is really important for me." What they carry is decided per kind, and each kind is a preference of its own: one switch labelled "notifications" is how people end up unsubscribing from the one thing they wanted in order to escape the rest. |
| FR-H2 | MUST | **Transactional and optional mail are separated, and only the optional kind is a preference.** A confirmation of an address change (FR-A13), a moderation outcome (FR-E) and a deletion receipt (FR-A15) are the platform doing what the member asked; suppressing them would hide from somebody that their account is being changed. Everything else is opt-in and one click from off, and the click does not require signing in, because a person who wants out and cannot get out complains to a regulator rather than to us. |
| FR-H3 | MUST NOT | **No notification may reveal the author of an anonymous contribution, to anybody, including its author.** "Your anonymous review was removed" requires knowing which review is whose, which is the link FR-C2 forbids. Anonymous moderation outcomes are published in place (FR-E9) and are not mailed. This is the one place FR-C constrains FR-H, and it constrains the *contents* of a message, never whether the platform may hold an address. |
| FR-H4 | MUST | **A module never receives an address and cannot send mail.** It asks the platform to notify a member it already has an id for, exactly as it asks for a username through a resolver today, so `studens_ryc` gains no grant on the member table. |
| FR-H5 | MUST | **Sending is queued and retried, never done inside a request.** A provider that is slow or down must not make a submission fail or hang, and a moderation decision must not be lost because mail bounced. The worker from 1.7 is the consumer. |
| FR-H6 | SHOULD | **Delivery is over SMTP with no vendor lock-in**, because zero budget rules out the paid tiers and every free tier lapses. The address of the relay is configuration; the code knows nothing about who provides it. |
...
| NFR-O4 | MUST | **Anything that can grow is given a strategy in the change that introduces it, not in the change that discovers it hurts.** A list, a page, a table, a query, a file, a queue: if the number of things in it is not fixed by construction, the change that adds it says how it behaves at ten times the current size, and does something about it then. This serves MEA directly (1.05): a screen that has to be rebuilt the first time it succeeds is neither maintainable nor evolvable, and the rebuild always lands at the worst moment, when somebody is finally using it. **It is not a performance requirement.** The failures here were legibility long before they were speed: 976 programmes rendered as a 143,112 pixel page, a search reporting "25 of 25" because 25 was the window, a moderation register that only grows, 349 courses in one programme. Each looked fine at the size it was written for. **Cost accepted:** a cap, a page or a step costs a control and a decision on every list, including lists that will never be large, and some of those controls will never be pressed. **What would change the answer:** nothing seen so far; the cheap version, drawing a screenful and offering the rest, has been right every time it has been applied. **[VERIFIED]** 2026-09-21. |
...
| ID | Constraint |
|---|---|
| CON-1 | **Zero budget.** No paid services or tiers, no trials that convert to charges. Free or open source and self-hostable only. Where a free tier is used, its limit and the exit path are documented. **[VERIFIED]** 2026-09-09. |
| CON-2 | **Team of two to three developers**, one of them part-time on operations. **[VERIFIED]** 2026-09-09. |
| CON-3 | **No deadline.** Time may be traded for quality. This forbids shortcuts justified by speed. **[VERIFIED]** 2026-09-09. |
| CON-4 | **Git is the coordination mechanism.** Shared history is not rewritten. **[VERIFIED]** 2026-09-09. |
...
**[VERIFIED]** 2026-09-10: **Oracle Cloud Always Free, for now.** Resolves OPEN-29.
...
cold starts. The call on 2026-09-10 was Oracle for now, so zero budget holds. The
...
| ~~OPEN-7~~ | ~~Is the 24 hour moderation target sustainable?~~ **RESOLVED 2026-09-13.** | closed |
| ~~OPEN-8~~ | ~~How are Moderators appointed?~~ **RESOLVED 2026-09-13.** | closed |
...
| ~~OPEN-18~~ | ~~Who governs contributions? Review authority and merge rights.~~ **RESOLVED 2026-09-11**, both halves: MIT on 2026-09-09, and governance on 2026-09-11 as FR-B19 and FR-B20, once a second developer made review by another person possible. | closed |
...
| ~~OPEN-24~~ | ~~Threshold for automatic removal versus holding for a human?~~ **RESOLVED 2026-09-13.** | closed |
...
| ~~OPEN-35~~ | ~~Open registration means one person can hold many accounts.~~ **RESOLVED 2026-09-13.** | closed |
...
| ~~OPEN-45~~ | ~~The catalogue contains courses taught at other institutions.~~ **RESOLVED 2026-09-13.** | closed |
| ~~OPEN-48~~ | ~~A course code in a URL does not say which university it belongs to.~~ **RESOLVED 2026-09-18: the institution goes in the path.** The owner's call, over keeping bare codes and disambiguating only on collision, which would have let one URL mean different things as the catalogue grew. `/app/ryc/c/uclouvain/lepl1503` and `/app/ryc/p/uclouvain/sinf1ba`. **The cost is that every link written before today carries no institution**, and it is paid rather than passed on: a bare code parses as its own kind, is looked up, and is forwarded to its real address with its query string intact. One match is forwarded; several is a genuinely ambiguous link and the screen offers the choice rather than guessing, because guessing sends a reader to a university they never asked about. The institution is also a browse FILTER, not a gate, on the same ground as the faculty in `design/catalogue-ingestion.md` 12.3, and a dimension with one option is not drawn, so the control appears only once a second catalogue exists. | closed |
| OPEN-47 | **The catalogue is French and the interface is not.** The crawl fetches the French edition, so every stored block is French whatever language a visitor chose. Marked on screen since 2026-09-17 rather than fixed. Fixing it means crawling `en-cours-...` as well (78 more minutes per run), storing a language per field, and falling back to French per field, because the two editions disagree about which fields exist: on 80 sampled courses, 53 publish an assessment in French and 38 in English. Dutch has no source at all, `nl-cours-...` is 404. | FR-D19, FR-G, `design/catalogue-ingestion.md` 12.15 |
| ~~OPEN-42~~ | ~~Permission to import the existing EPL reviews document.~~ **RESOLVED 2026-09-18: not asked, and not imported.** The decision was to stay unaffiliated and out of any administrative process for now, accepting that the first students arrive at a product that is mostly new in review terms. The cost is named rather than softened: on the day this was decided the catalogue held 6,715 courses and 13 reviews, so the first cohort arrives at an almost empty product and the FR-D17 import path has no content to carry. Accepted in exchange for owing nothing to a faculty, asking no administration for anything, and having no third party able to withdraw permission later. FR-D18 stands and blocks nothing, because nothing is being imported. | closed |
...
| ~~OPEN-46~~ | What happens to a member's **attributed** contributions when they delete their account (FR-A15). | Three answers, all defensible, and the choice is the owner's. **Delete them**: cleanest reading of Article 17, and it removes reviews other students are relying on, which is content loss the platform exists to prevent. **Detach them**, keeping the text and dropping the name: Recital 26 makes properly anonymised data fall outside the GDPR, so this is lawful, and it quietly moves a contribution from the attributed path to something that looks anonymous without its author ever choosing that, which sharpens the complement problem in 3.3 against everyone else. **Keep them under the name**, on the ground that a signed public statement is not withdrawn by closing an account: defensible, and the hardest to argue to a regulator. **RESOLVED 2026-09-13: detach them.** The owner chose the second. Built as a FOURTH state and not as a second anonymous one, which is what makes it safe: a detached contribution keeps the numbers it was published with, is labelled as coming from a deleted account, and is counted with neither the named nor the anonymous set. Folding it into the anonymous count would inflate the figure FR-C21 puts in front of the next contributor, so the number somebody uses to judge their own exposure, and 3.3's arithmetic with it, would be wrong. Raised 2026-09-13. |
...
| OPEN-18 (licence part) | Which licence? | ~~**MIT.** Permissive, chosen over AGPL with the trade-off understood: anyone may host a derivative commercially without contributing back, in exchange for the lowest possible barrier to the outside contributors the vision depends on (1.0). Inbound equals outbound, so no contributor licence agreement is needed. Resolved 2026-09-09.~~ **REVERSED 2026-09-19: AGPL-3.0-or-later.** The requirement is unchanged, which is why this is a reversal and not a new question: the licence exists to let people read, check and build on this, and the anonymity promise leans on the code being readable (1.1, FR-C12). **What changed is that the exchange did not pay.** The barrier was lowered for outside contributors who have not arrived: `git shortlog -sne --all` lists ONE author 112 commits in, so the benefit bought was zero, while the cost accepted, that anyone may run a derivative commercially without contributing anything back, is the one the owner was not willing to carry: this is a long-term project, and even work meant to help people cannot assume everyone can be trusted with it. **Alternatives considered now:** staying MIT, rejected because it grants precisely the thing feared; going PRIVATE, rejected for two reasons, that it retracts nothing already published and that it would falsify the product's own claim that the code is checkable, which is what makes the anonymity promise verifiable rather than merely asserted; a dual licence, rejected as machinery for a problem one person does not have. **Cost accepted:** the AGPL deters some contributors and some reuse, and it is a copyleft licence in a world that mostly ships permissive ones. **What would change the answer:** outside contributors turning up and saying the licence is what stops them, which is the same evidence that was assumed in 2026-09-09 and never appeared. The MIT grant on everything published before 2026-09-19 is irrevocable and is not being pretended away; see `NOTICE`. |
...
**Studea collision, CONFIRMED 2026-09-09.** *Studéa* and `studea.fr` already
...
- Project context supplied by the owner, 2026-09-09 and 2026-09-10. **Authoritative.**
